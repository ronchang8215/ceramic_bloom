...
<div>
  ...
  <div class="mt-3 text-right">
    ...
    @click.prevent="openModal(true)">建立新優惠券</button>
  </div>
...
export default {
  ...
  methods: {
    ...
    getCoupon(page = 1) {
      const vm = this;
      vm.isLoading = true;
      this.$http.get(`${process.env.VUE_APP_APIPATH}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/coupons?page=${page}`).then((res) => {
    ...
    updateCoupon() {
      ...
      this.$http[httpMethod](api, { data: vm.tempCoupon }).then((res) => {
        ...
        if (res.data.success) {
          ...
          $('#couponModal').modal('hide');
          vm.getCoupon();
          this.$bus.$emit('message:push', '新增優惠券失敗', 'danger');
        }
      });
    },
    deleteCoupon() {
      const vm = this;
      this.$http.delete(`${process.env.VUE_APP_APIPATH}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/coupon/${vm.tempCoupon.id}`).then((res) => {
        console.log(res.data);
        if (res.data.success) {
          ...
          $('#delCouponModal').modal('hide');
          vm.getCoupon();
          this.$bus.$emit('message:push', '刪除優惠券失敗', 'danger');
        }
      });
    },
  },
